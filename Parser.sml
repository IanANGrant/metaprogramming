...
type nonterm = int
type context = (nonterm * Substring.substring) list
...
fun addnt ctxt (p' as (nt,sb)) =
    let val n = Substring.size sb
        fun copyout r [] = rev r
          | copyout r (h::t) = copyout (h::r) t
        fun iter r [] = rev ((nt,sb)::r)
          | iter r (l as ((p as (nt',sb'))::ps)) =
               let val n' = Substring.size sb'
               in if nt < nt'
                  then copyout ((nt,sb)::r) l
                  else (if nt = nt'
                        then (if n > n'
                              then iter (p::r) ps
                              else copyout ((nt,sb)::r) l)
                        else iter (p::r) ps)
               end
    in
       iter [] ctxt
...
fun update_lctxt nt (p: 'a parser) =
   fn ({lc,sb}:input) => p {lc=addnt lc (nt,sb),sb=sb}
...
(* Context equivalence is used to decide whether a result in the cache
   ...
   with c=((nt_1,(i_1,n_1)),(nt_2,(i_2,n_2)),...) and
   c'=((nt'_1,(i'_1,n'_1)),(nt'_2,(i'_2,n'_2)),...) are equivalent, as
   far as a parser given an input i'',n'' is concerned, when the
   sub-sequences of the nonterminals nt_j and nt'_k such that
   (i'',n'')=(i_j,n_j)=(i'_k,n'_k) are permutations of each
   other. This is more precisely explained in Standard ML than it
   could be in `English'! *)

fun eqv_lctxts (l,n) =
   let fun empty [] = true
         | empty ((_,(l',n'))::t) =
              (not (l=l' andalso n=n')) andalso empty t
       fun equiv [] [] = true
         | equiv [] (c2 as (_::t2)) = empty c2
         | equiv (c1 as (_::t1)) [] = empty c1
         | equiv (c1 as ((nt,(l',n'))::t1))
                 (c2 as ((nt',(l'',n''))::t2)) =
               if l=l' andalso n=n'
               then if l=l'' andalso n=n''
                    then nt=nt' andalso equiv t1 t2
                    else equiv c1 t2
               else if l=l'' andalso n=n''
                    then equiv t1 c2
                    else equiv t1 t2
   in equiv
   end

(* We only save a stripped-down version of the context in the cache,
   because the cache does not need to record the actual string
   contents, only the start and end positions. *)

fun stripctxt r [] = rev r
  | stripctxt r ((nt,sb)::ps) =
        let val (_,i,n) = Substring.base sb
        in stripctxt ((nt,(i,n))::r) ps
        end
...
type 'a memo = (((int * (int * int)) list * (int * int)) * ('a * substring) list) ml ref
...
fun memop pad p:'a parser = 
   ...
   in
      fn (i as ({lc,sb}:input)) =>
           mem (fn () => p i) (stripctxt [] lc) (stripss sb) pad
   end
...
fun ctxteq (nt,s) (nt',s') =
   nt = nt' andalso subseqind s s'

fun check_and_upd_lctxt nt (p:'a parser) =
   fn (i as ({lc,sb}:input)) =>
      let val should_trim =
            List.exists (fn x => ctxteq x (nt,sb)) lc
      in
         if should_trim andalso len sb = 0
         then []
         else if should_trim 
              then (ignr_last (update_lctxt nt p)) i
              else (update_lctxt nt p) i
      end
